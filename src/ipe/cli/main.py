...
def generate(
    ...
    try:
        ...
        if not resolved.spec_path:
            ...
            console.info(
                "Pass a spec ('ipe generate api.yaml -o ./out') or run 'ipe init'"
            )
...
def init(
    ...
    suggested = to_snake_case(parsed.info.title) if parsed else "api_client"
    if not _is_valid_module(suggested):
        suggested = "api_client"

    module_name = _ask_module_name(suggested)
...
def _ask_module_name(suggested: str) -> str:
    while True:
        name = console.ask("Module name", default=suggested)
        if _is_valid_module(name):
            return name
        console.error(
            f"'{name}' is not a valid module name — use letters, numbers, and "
            "underscores, and don't start with a digit or use a Python keyword."
        )
